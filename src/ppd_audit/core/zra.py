...
def throttle_loss(
    *,
    ...
    ΔP_гидр = Δp_задв·Q/3.6 (31);  ΔP_др = ΔP_гидр/η_факт (32);
    ΔW_др = (Δp_задв)/(3.6·η_факт)·Q_год (45).
    """
    dp = p_before - p_after
    p_hyd = units.hydraulic_power_kw(dp, q)  # (31)
    p_el = p_hyd / eta_fact  # (32)
    q_year = q * t_year
    annual = dp / (units.HYDRAULIC_POWER_DIVISOR * eta_fact) * q_year
    return ThrottleLoss(
        dp_throttle=dp,
        power_hydraulic=p_hyd,
        power_electric=p_el,
        annual_kwh=annual,
        annual_rub=annual * tariff,
    )
