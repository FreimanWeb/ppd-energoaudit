...
class RegimeResult:
    ...
    # доп. для КНС
    p_bg: float | None = None  # давление на БГ, МПа
    dp_valve: float | None = None  # Δp на задвижке, МПа (9)
    h_bg: float | None = None  # напор до БГ, м (10)
...
def decompose_pumping(
    regime: RegimeResult,
    *,
    eta_motor_nom: float,
    eta_motor_real: float,
    eta_due: float,
    eta_due_visc: float | None = None,
    eta_vfd: float = 1.0,
    eta_gear: float = 1.0,
    ...
    return PumpingDecomposition(
        ...
        dp_efficiency=dp_kpd,
        dp_suboptimal=dp_suboptimal,
        dp_viscosity=dp_visc,
        dp_motor=dp_motor,
        dp_wear=dp_wear,
        eta_pump=eta_pump,
        balance_residual=residual,
        balance_ok=abs(residual) < tol,
    )
...
def decompose_kns(regime: RegimeResult, *, eta_nom: float | None = None) -> KnsDecomposition:
