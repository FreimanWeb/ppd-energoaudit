...
class Measure:
    ...
    # Показатель степени зависимости эффекта от объёма закачки: эффект года k
    # = базовый × (Q_год(k)/Q_год.база)**volume_exponent. Используется только
    # в measures/economics.py при расчёте на горизонте по прогнозу закачки.
    # 1.0 — эффект пропорционален перекачанному объёму (ΔP ∝ Q, ΔW = ΔP·T_год);
    # 3.0 подошло бы мероприятию по трению в трубопроводе (ΔW ∝ Q³);
    # 0.0 — эффекту, не зависящему от подачи.
