...
class PumpPassport(BaseModel):
    ...
    power_consumed_nom: float | None = Field(None, description="потребляемая мощность, кВт")
    overhaul: str | None = None
...
class Aggregate(BaseModel):
    ...
    pump: PumpPassport | None = None
    motor: MotorPassport | None = None
...
class RegimeInputs(BaseModel):
    ...
    w: float | None = Field(None, description="расход ЭЭ за период W, кВт·ч")
    t: float | None = Field(None, description="время работы T за период, ч")
    p_electric: float | None = Field(None, description="активная мощность P_эл, кВт")

    q_day: float | None = Field(None, description="суточный объём Q_сут, м³")
    q_year: float | None = Field(None, description="годовой объём Q_год, м³")
    ...
    p_opt: float | None = Field(None, description="оптимальное давление p_опт, МПа")
...
class Plant(BaseModel):
    ...
    reference_regime: dict | None = None
