...
def render(ctx: Ctx) -> None:
    ...
    if volumetric:
        st.caption(
            "Плунжерный насос: паспортная кривая Q-H неприменима, показаны факт. точки."
        )
        ...
        if h_due:
            fig.add_trace(
                go.Scatter(
                    ...
                    y=[h_due],
                    mode="markers",
                    name="Должный напор (29)",
                    marker=dict(size=12, color="#e0a106"),
                )
            )
        fig.add_trace(
            go.Scatter(
                ...
                name="Факт",
                marker=dict(
                    size=16, color="#d9534f", symbol="x", line=dict(width=2, color="#9c2b27")
                ),
            )
        )
        fig.update_layout(
            title="Q–H: насос и трубопровод",
            xaxis_title="Q, м³/ч",
            yaxis_title="H, м",
            height=420,
    ...
    with cc[1]:
        ...
        if p.curve_qeta:
            ...
            fig.add_trace(
                go.Scatter(
                    x=qe,
                    y=p.eta_nom * (1 - 0.9 * ((qe - p.q_nom) / p.q_nom) ** 2),
                    mode="lines",
                    name="Q-η · модель",
        ...
        st.plotly_chart(fig, width="stretch")
    st.caption(
        f"Рабочая подача Q = {fmt(q_work, 2)} м³/ч · напор факт {fmt(reg.h_fact, 1)} м · "
