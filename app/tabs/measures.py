...
* «текущий режим» — как раньше: годовой эффект текущего режима, окупаемость
...
def render(ctx: Ctx) -> None:
    ...
    basis = st.radio(
        ...
        key="measures_basis",
        help=(
            "«Текущий режим» — эффект сегодняшнего режима, повторённый на год. "
...
def _render_horizon_economics(ctx: Ctx, *, default_horizon: int) -> None:
    ...
    if profile is None:
        st.warning(
            "Прогноз закачки не построен: мало данных. "
            "Экономика посчитана при постоянном объёме закачки."
        )
        profile = InjectionProfile.flat(base_annual, horizon)

    st.caption(
        "Эффект каждого года масштабируется прогнозным объёмом закачки. "
        "Ориентир для ранжирования, не проектное ТЭО."
    )
    ...
    st.caption(
        f"«> горизонта» — CAPEX не окупился за {horizon} лет. "
        "IRR не считается для мероприятий без CAPEX."
    )
