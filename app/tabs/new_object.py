...
def render(ctx: Ctx) -> None:
    ...
    st.caption(
        "Откуда модель берёт данные: телеметрия (автоматически) + паспорта/конфиг "
    ...
    st.caption(
        "Полный контракт с приоритетами P0/P1/P2 и заменами при отсутствии — "
    ...
    checks = [
        ("Телеметрия: p_вх", rm_n.p_in),
        ("Телеметрия: p_вых", rm_n.p_out),
        ("Телеметрия: p_БГ (КНС)", rm_n.p_bg),
        ("Телеметрия: Q_сут", rm_n.q_day),
        ("Телеметрия: W (ЭЭ/сут)", rm_n.w),
        ("Телеметрия: T (наработка)", rm_n.t),
        ("Плотность ρ", rm_n.rho),
        ("Паспорт насоса: H_ном", p_n.h_nom),
        ("Паспорт насоса: Q_ном", p_n.q_nom),
        ("Паспорт насоса: η_ном", p_n.eta_nom),
        ("Кривая Q-H", p_n.curve_qh if p_n.curve_qh else None),
        ("Паспорт ЭД: P_ном", m_n.p_nom),
        ("Паспорт ЭД: η_ном", m_n.eta_nom),
    ]
    ...
    st.dataframe(
        pd.DataFrame([
            {"Поле": k, "Статус": "✓ есть" if v else "— нет", "Значение": _val(v)}
            for k, v in checks
        ]),
        width="stretch",
        hide_index=True,
    ...
    if missing:
        st.warning(
            "Не заполнено: " + ", ".join(missing) + ". Часть полей опциональна "
    ...
    st.markdown(
        "Затем выполнить (сконвертирует `.doc`, создаст `plants/<id>.yaml`, "
