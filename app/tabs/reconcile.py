...
def _fmt_report(r) -> str:
    ...
    if r["report"] is None and r["report_lo"] is None:
        return "—"
    if r["report_lo"] is not None and r["report_hi"] is not None \
...
def render(ctx: Ctx) -> None:
    ...
    if not rows:
        st.info("Для этого объекта нет текстового отчёта: у ДНС-7с эталон — телеметрия и "
        ...
        def build_df(rs):
            return pd.DataFrame([{
                ...
                "расчет.xlsx": fmt(r["xlsx"], 3), "Отчёт .doc": _fmt_report(r),
